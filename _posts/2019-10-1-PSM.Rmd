...
```{r echo = FALSE}
FinEdWide <- as.data.frame(read.csv("chess.csv"))
summary(FinEdWide)
FinEdLong <- tidyr::gather(FinEdWide, PrePost, Answers, c(Pre,Post), factor_key=TRUE)

FinEdWide$Performance <- relevel(FinEdWide$Performance,  "Satisfactory" )
FinEdWide$Performance <-relevel(FinEdWide$Performance,  "Excellent" )
```

We have `r dim(FinEdWide)[2]` variables and `r dim(FinEdWide)[1]` observations without any missing values. The variable `Treatment.Type` shows the group (Control or Treatment) to which students were randomly assigned. The variables `Pre` and `Post` show the number of correct answers out of 40 logical questions. Last 4 variables are the characteristics of the students: the gender, age, math grade of a student for the last semester, and overall performance by the end of the semester.    
...
```{r echo=FALSE}
ggplot(FinEdLong, aes(x = PrePost, y = Answers, col = PrePost))+
    geom_boxplot(alpha = 0.8) +
  xlab(" ") + scale_color_brewer(palette="Set1")+
...
```{r echo=FALSE}
summary(lm(Answers ~ Treatment.Type + PrePost + Treatment.Type * PrePost, data = FinEdLong))
```
...
<center>![The backdoor path between treatmnent and outcome variable](PsmConfounding.png){width=20%}</center>
...
for (i in 1:length(indexcat)){
  
      var = c("Gender", "Performance")
      
      indexcat[i] = grep(var[i], colnames(FinEdWide))
      
      chisq[i] = getElement(chisq.test(table(FinEdWide$Treatment.Type,
        FinEdWide[,as.numeric(indexcat[i])])), "statistic")
      
      pval[i] = getElement(chisq.test(table(FinEdWide$Treatment.Type,
        FinEdWide[,as.numeric(indexcat[i])])), "p.value")
     
}

# Randomization check by tests
table1 <- data.frame( "Variable" = c("Gender", "Overall Performance"),
                      "ChiSquare" = round(as.numeric(chisq),3), 
                      "P-value" =# substr(format(
                        as.numeric(pval)
  #                      ,scientific = FALSE), 1,17)
  )
table1
...
```{r echo=FALSE}
mathan <- summary(aov(FinEdWide[,"Math.grade"]~Treatment.Type, data = FinEdWide))
agean <- summary(aov(FinEdWide[,"Age"]~Treatment.Type, data = FinEdWide))
fval <- c(mathan[[1]]$`F value`[1], agean[[1]]$`F value`[1])
pvala <- c(mathan[[1]]$`Pr(>F)`[1], agean[[1]]$`Pr(>F)`[1])
(table2 <- data.frame( "Variable" = c("Math.grade", "Age"),
                      "F" = round(as.numeric(fval),3), 
                      "P-value" = substr(format(
                        as.numeric(pvala)
                        ,scientific = FALSE), 1,13)
  ))
...
g1 <- ggplot(FinEdWide,aes(x = Gender, fill = Gender)) +
    geom_bar(position = "dodge") + 
    geom_text(data = tablecat(Gender), size = 3.5, aes(y = n + 6, label=paste(Percentage, "%")),
        position =  position_dodge(width = 1))+
    xlab(" ") + ylab("Count")+
  scale_fill_brewer(palette="Set1") +
  ggthemes::theme_pander() +
  ggtitle("Gender by the treatment type")+
    labs(fill = "Gender")+
  theme(legend.position = "None")+
  facet_grid(.~Treatment.Type)
g2 <- ggplot(FinEdWide, aes(x = Treatment.Type, y = Math.grade, fill = Treatment.Type)) +
  geom_boxplot(alpha = 0.9) +
  stat_summary(fun.y=mean, geom="point", shape=20, size=5, color="black", fill="white")+ 
  xlab(" ") +
  labs(col = "Treatment type")+
  ggtitle("Math grade by the treatment \n type") +
   scale_fill_brewer(palette="Set1") +
  ggthemes::theme_pander() +
   theme(legend.position="none")
...
```{r echo=FALSE}
# Age
g3 <- ggplot(FinEdWide, aes(x = Treatment.Type, y = Age, fill = Treatment.Type)) +
  geom_boxplot(alpha = 0.9) +
  stat_summary(fun.y=mean, geom="point", shape=20, size=5, color="black", fill="white")+ 
  xlab(" ") +
  labs(col = "Treatment type")+
  ggtitle("Age by the treatment \n type") +
   scale_fill_brewer(palette="Set1") +
  ggthemes::theme_pander() +
   theme(legend.position="none")
...
```{r echo=FALSE}
g4 <- ggplot(FinEdWide,aes(x  = Performance, fill = Performance)) +
    geom_bar(position = "dodge") + 
    geom_text(data = tablecat(Performance), size = 3, aes(y = n + 6, label=paste(Percentage, "%")),
        position =  position_dodge(width = 1))+
    xlab(" ") + ylab("Count")+
  scale_fill_brewer(palette="Set1") +
  ggthemes::theme_pander() +
  ggtitle("Overall Performance by the \n treatment type")+
 theme(legend.position = "None")+
  facet_grid(.~Treatment.Type)+
...
```{r echo=FALSE}
table(FinEdWide$Treatment.Type)
```
...
g5 <- ggplot(matcheddata,aes(x = Gender, fill = Gender)) +
    geom_bar(position = "dodge") + 
    geom_text(data = tablecat1(Gender), size = 3.5, aes(y = n + 6, label=paste(Percentage, "%")),
        position =  position_dodge(width = 1))+
    xlab(" ") + ylab("Count")+
...
g6 <- ggplot(matcheddata, aes(x = Treatment.Type, y = Math.grade, fill = Treatment.Type)) +
  geom_boxplot(alpha = 0.9) +
  stat_summary(fun.y=mean, geom="point", shape=20, size=5, color="black", fill="white")+ 
  xlab(" ") +
  labs(col = "Treatment type")+
...
```{r echo=FALSE}
# Age
g7 <- ggplot(matcheddata, aes(x = Treatment.Type, y = Age, fill = Treatment.Type)) +
  geom_boxplot(alpha = 0.9) +
  stat_summary(fun.y=mean, geom="point", shape=20, size=5, color="black", fill="white")+ 
  xlab(" ") +
  labs(col = "Treatment type")+
  ggtitle("Age grade by the treatment \n type") +
   scale_fill_brewer(palette="Accent") +
  ggthemes::theme_pander() +
   theme(legend.position="none")
```
...
```{r echo=FALSE}
# Overall performance
g8 <- ggplot(matcheddata,aes(x  = Performance, fill = Performance)) +
    geom_bar(position = "dodge") +
    geom_text(data = tablecat1(Performance), size = 3, aes(y = n + 6, label=paste(Percentage, "%")),
        position =  position_dodge(width = 1))+
    xlab(" ") + ylab("Count")+
  scale_fill_brewer(palette="Accent") +
  ggthemes::theme_pander() +
